...
int tcp_sendmsg_locked(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	flags = msg->msg_flags;
	if ((flags & MSG_FASTOPEN || inet_sk(sk)->defer_connect) && !tp->repair) {
...
struct sk_buff *tcp_get_timestamping_opt_stats(const struct sock *sk)
{
	...
	tcp_get_info_chrono_stats(tp, &info);
	nla_put_u64(stats, TCP_NLA_BUSY,
			  info.tcpi_busy_time);
	nla_put_u64(stats, TCP_NLA_RWND_LIMITED,
			  info.tcpi_rwnd_limited);
	nla_put_u64(stats, TCP_NLA_SNDBUF_LIMITED,
