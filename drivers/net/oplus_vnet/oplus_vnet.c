...
#include <net/arp.h>       /* if you use arp_tbl */
#include <net/neighbour.h>  /* for neigh_output */
...
static void ovnet_setup(struct net_device *dev)
{
	...
	dev->destructor = ovnet_dev_free;
}


/*
 * Linux 4.4 适配:
 * 原始代码的 validate/newlink/changelink 使用了 netlink_ext_ack 参数，
 * 该参数在 Linux 4.12 中才引入。此 4.4 内核的 rtnl_link_ops 使用旧签名：
 *   - validate:  没有 extack 参数
 *   - newlink:   没有 extack 参数
 *   - changelink: 没有 extack 参数
 */


static int ovnet_validate(struct nlattr *tb[], struct nlattr *data[])
{
	logk("cell_validate %p %p", tb, data);
	return 0;
}


static int ovnet_newlink(struct net *src_net, struct net_device *dev,
			 struct nlattr *tb[], struct nlattr *data[])
{
	struct ovnet_dev_private *private = netdev_priv(dev);
	int ret = 0;

	logk("ovnet_newlink ...%s %p %p", dev->name, tb, data);
	if (!data) {
		return 0;
	}
			    ...
			    struct nlattr *data[])
{
	...
	logk("ovnet_changelink ....%s %p %p", dev->name, tb, data);
	if (!data) {
		return 0;
	}
...
static int init_dev_one(int index)
{
	...
	dev_vnet = alloc_netdev(sizeof(struct ovnet_dev_private), "ovnet%d",
...
static int __init ovnet_dev_init(void)
{
	...
	s_ovnet_table_header = register_net_sysctl(&init_net, "net/ovnet_dev",
						    s_ovnet_dev_sysctl_table);
